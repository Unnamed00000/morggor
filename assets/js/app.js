(function () {
  ...
  const BACKEND_STATUS_URL = "/api/status";
  const BACKEND_SESSION_URL = "/api/session";
  const BACKEND_LOGIN_URL = "/api/login";
  const BACKEND_LOGOUT_URL = "/api/logout";
  const BACKEND_TREE_URL = "/api/tree";
  ...
  function savePeople() {
    ...
    if (backendAvailable && isEditUnlocked()) {
      queueBackendSave();
    }
  }
  ...
  function queueBackendSave() {
    ...
    backendSaveTimer = window.setTimeout(() => savePeopleToBackend(snapshot), 300);
  }

  async function savePeopleToBackend(snapshot) {
    try {
      const response = await fetch(BACKEND_TREE_URL, {
        method: "PUT",
      ...
      if (response.status === 401) {
        ...
        alert("Сессия закончилась. Войдите ещё раз, чтобы сохранить изменения.");
        return;
      }

      if (!response.ok) throw new Error("Cannot save to backend");
      backendSaveWarningShown = false;
    } catch (error) {
      console.warn("Cannot save family tree to backend", error);
      if (!backendSaveWarningShown) {
        backendSaveWarningShown = true;
        alert("Не получилось сохранить данные на сервер. Проверьте, что backend запущен.");
      }
    }
  }

  async function loadPeopleFromBackend(silent = false) {
    try {
      const response = await fetch(BACKEND_TREE_URL, { cache: "no-store" });
      if (!response.ok) throw new Error("Cannot load backend data");
      const data = await response.json();
      const loadedPeople = Array.isArray(data) ? data : data.people;
      if (!Array.isArray(loadedPeople)) throw new Error("Wrong backend data format");
      people = loadedPeople.map(normalizePerson);
      localStorage.setItem(STORAGE_KEY, JSON.stringify(people));
      placeMissingPeople();
      render();
    } catch (error) {
      console.warn("Cannot load family tree from backend", error);
      if (!silent) {
        alert("Не получилось загрузить общее дерево с сервера.");
      }
    }
  }
  ...
  async function initBackend() {
    ...
    try {
      const treeResponse = await fetch(BACKEND_TREE_URL, { cache: "no-store" });
      if (!treeResponse.ok) {
        await loadPublishedTreeFallback();
        return;
      }

      backendAvailable = true;
      fetch(BACKEND_STATUS_URL, { cache: "no-store" }).catch(() => {});

      const session = await fetch(BACKEND_SESSION_URL, { cache: "no-store" });
      if (session.ok) {
        const data = await session.json();
        setEditUnlocked(Boolean(data.unlocked));
      } else {
        setEditUnlocked(false);
      }
  ...
  async function loadPublishedTreeFallback() {
    try {
      const response = await fetch("data/rodoslovnoe-derevo-data.json", { cache: "no-store" });
      if (!response.ok) return;
      const data = await response.json();
      const loadedPeople = Array.isArray(data) ? data : data.people;
      if (!Array.isArray(loadedPeople) || !loadedPeople.length) return;
      people = loadedPeople.map(normalizePerson);
      localStorage.setItem(STORAGE_KEY, JSON.stringify(people));
      placeMissingPeople();
      render();
    } catch (error) {
      console.warn("Cannot load published tree fallback", error);
    }
  }
  ...
  function isEditUnlocked() {
    if (backendAvailable) return backendLoggedIn;
    return sessionStorage.getItem(AUTH_SESSION_KEY) === "1";
  }

  function setEditUnlocked(unlocked) {
    if (backendAvailable) {
      backendLoggedIn = unlocked;
    }
    document.body.classList.toggle("edit-locked", !unlocked);
    document.body.classList.toggle("viewer-mode", !unlocked);
    if (authButton) {
  ...
  async function handleAuthButton() {
    if (isEditUnlocked()) {
      await logoutEditMode();
      return;
    }
    await requestEditLogin();
  }

  async function requestEditLogin() {
    ...
    if (backendAvailable) {
      try {
        ...
        if (response.ok) {
          ...
          await loadPeopleFromBackend(true);
          render();
          return;
        }
      } catch (error) {
        console.warn("Cannot login to backend", error);
        alert("Backend не отвечает. Проверьте, что сервер запущен.");
        return;
      }
  ...
  async function logoutEditMode() {
    if (backendAvailable) {
      try {
        await fetch(BACKEND_LOGOUT_URL, { method: "POST" });
      } catch (error) {
        console.warn("Cannot logout from backend", error);
      }
